...
class GadflyWebSocketComm {
  ...
  call_alert() {
    alert("in wcomm gadfly.jsx");

    this.wcomm.call_alert();
  }
  debouncedSend500(keys, data) {
    this.addPendingData(keys, data);
    this.debouncedSendPendingData500();
...
class IOPairChart extends React.Component {
  constructor(props) {
    ...
    // this.comm.subscribe('finish_update_alert', this.finish_update_alert)
    this.comm.callbackMap['finish_update_alert'] = this.finish_update_alert
        ...
        {!this.state.read_only && <div className={this.state.suggestionsDropHighlighted ? "gadfly-drop-highlighted gadfly-suggestions-box" : "gadfly-suggestions-box"} style={{paddingTop: "39px"}} onDragOver={this.onSuggestionsDragOver} onDragEnter={this.onSuggestionsDragEnter}
         onDragLeave={this.onSuggestionsDragLeave} onDrop={this.onSuggestionsDrop}>
          ...
          <div style={{position: "absolute", top: "10px", width: "100%"}}>
            ...
            &nbsp;&nbsp;&nbsp;<span onClick={this.clickButton1} style={{cursor: "pointer"}}>Update Local</span>&nbsp;&nbsp;&nbsp;<span  style={{cursor: "pointer"}} onClick={this.clickButton2}>Update Global</span>
  ...
  finish_update_alert(data) {
    alert("The update is finished, you can now start working again :-)")
  }
  ...
  clickButton1(e) {
    alert('coalign is going to update the local model now, you should not do any other actions until the update is done!'+
    'click OK and another alert shows up when the update is done!')
    e.preventDefault();
    e.stopPropagation();
    console.log("clickButton1");
    this.comm.send(this.id, {
      action: "button1", value2_filter: this.state.value2Filter, value1_filter: this.state.value1Filter,
      comparator_filter: this.state.comparatorFilter,
      suggestions_template_value1: this.suggestionsTemplateRow && this.suggestionsTemplateRow.state.value1,
      suggestions_template_comparator: this.suggestionsTemplateRow && this.suggestionsTemplateRow.state.comparator,
      suggestions_template_value2: this.suggestionsTemplateRow && this.suggestionsTemplateRow.state.value2,
...
clickButton2(e) {
    alert('coalign is going to update the global model now, you should not do any other actions until the update is done!'+
    'click OK and another alert shows up when the update is done! \n (depending on the number of the examples, this may take a while)')
    e.preventDefault();
    e.stopPropagation();
    console.log("clickButton2");
    this.comm.send(this.id, {
      action: "button2", value2_filter: this.state.value2Filter, value1_filter: this.state.value1Filter,
      comparator_filter: this.state.comparatorFilter,
      suggestions_template_value1: this.suggestionsTemplateRow && this.suggestionsTemplateRow.state.value1,
      suggestions_template_comparator: this.suggestionsTemplateRow && this.suggestionsTemplateRow.state.comparator,
      suggestions_template_value2: this.suggestionsTemplateRow && this.suggestionsTemplateRow.state.value2,
